...
def jobs_page() -> HTMLResponse:
    ...
    if not cards:
        # FIX: was a plain string — {search_form(...)} was rendered as
        # literal text in the browser instead of the actual search widget.
        cards = f"""
...
async def run_pipeline_ui(request: Request) -> RedirectResponse:
    ...
    clear_cancel_flag()
    # FIX: calling blocking sync code directly inside async def froze the
    # entire event loop — /api/progress polls were never answered, so the
    # progress bar stayed at 0% for the whole search. asyncio.to_thread
    # runs the pipeline in a thread pool, keeping the event loop free.
    await asyncio.to_thread(lambda: DailyPipeline(config_path=config_path()).run(max_jobs=100))
    return RedirectResponse("/dashboard", status_code=303)
...
def status() -> Dict[str, Any]:
    config = load_config()
    status_path = Path(config["output"].get("summary_dir", "data_folder/output")) / "automation_status.json"
